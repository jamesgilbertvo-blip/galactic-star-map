...
# The secret key is now read from an environment variable for security.
# In a production environment, you must set this variable.
# For local testing, it falls back to the insecure dev key.
app.secret_key = os.environ.get('SECRET_KEY', 'super-secret-key-for-dev')

CORS(app, supports_credentials=True)

# API endpoints for the game (unchanged)
...
def get_db_connection():
    conn = sqlite3.connect(DATABASE_FILE); conn.row_factory = sqlite3.Row; return conn
...
def sync_faction_database(current_system_data, systems_data, wormholes_data, structures_data, faction_id):
    conn = get_db_connection(); cursor = conn.cursor(); all_systems = {}
    ...
    if structures_data and current_system_data and 'system' in current_system_data:
        ...
        cursor.execute("UPDATE systems SET catapult_radius = ? WHERE id = ?", (max_catapult_radius, current_system_id))
...
def sync_data():
    ...
    user_id = session['user_id']; conn = get_db_connection(); user = conn.execute('SELECT api_key, faction_id FROM users WHERE id = ?', (user_id,)).fetchone(); conn.close()
    ...
    conn = get_db_connection(); cursor = conn.cursor(); faction_row = conn.execute('SELECT id FROM factions WHERE name = ?', (faction_name,)).fetchone()
    if faction_row: faction_id = faction_row['id']
    else: cursor.execute('INSERT INTO factions (name) VALUES (?)', (faction_name,)); faction_id = cursor.lastrowid; conn.commit()
    if user['faction_id'] != faction_id:
        cursor.execute('UPDATE users SET faction_id = ? WHERE id = ?', (faction_id, user_id)); conn.commit(); session['faction_id'] = faction_id
    conn.close()
...
def register():
    ...
    try:
        cursor = conn.cursor(); faction = conn.execute('SELECT id FROM factions WHERE name = ?', (faction_name,)).fetchone()
        ...
        cursor.execute("INSERT INTO users (username, password, api_key, faction_id) VALUES (?, ?, ?, ?)", (username, password, api_key, faction_id)); user_id = cursor.lastrowid; conn.commit()
...
def login():
    ...
    conn = get_db_connection(); user = conn.execute('SELECT * FROM users WHERE username = ?', (username,)).fetchone(); conn.close()
...
def profile():
    ...
    if request.method == 'GET':
        user = conn.execute('SELECT api_key FROM users WHERE id = ?', (user_id,)).fetchone(); conn.close()
        return jsonify({'api_key': user['api_key'] if user else ''})
    elif request.method == 'POST':
        data = request.get_json(); updates, params = [], []
        if data.get('api_key'): updates.append("api_key = ?"); params.append(data.get('api_key'))
        if data.get('password'): updates.append("password = ?"); params.append(data.get('password'))
        if not updates: return jsonify({'message': 'No changes provided.'}), 400
        params.append(user_id); query = f"UPDATE users SET {', '.join(updates)} WHERE id = ?"; cursor = conn.cursor(); cursor.execute(query, tuple(params)); conn.commit(); conn.close()
...
@admin_required
def get_all_systems():
    conn = get_db_connection(); systems_list = conn.execute('SELECT id, name, position, catapult_radius FROM systems ORDER BY position ASC').fetchall(); conn.close()
    return jsonify([dict(row) for row in systems_list])
...
def update_system():
    ...
    conn = get_db_connection(); conn.execute('UPDATE systems SET catapult_radius = ? WHERE id = ?', (new_radius, system_id)); conn.commit(); conn.close()
    return jsonify({'message': f'System {system_id} updated successfully.'})
@app.route('/api/admin/wormholes')
@admin_required
def get_all_wormholes():
    conn = get_db_connection(); wormholes_list = conn.execute('SELECT s1.name as name_a, s2.name as name_b, w.system_a_id, w.system_b_id FROM wormholes w JOIN systems s1 ON w.system_a_id = s1.id JOIN systems s2 ON w.system_b_id = s2.id').fetchall(); conn.close()
    return jsonify([dict(row) for row in wormholes_list])
...
def add_wormhole():
    ...
    conn = get_db_connection()
    try:
        cursor = conn.cursor(); cursor.execute('INSERT INTO wormholes (system_a_id, system_b_id) VALUES (?, ?)', (min(id_a, id_b), max(id_a, id_b))); conn.commit()
    except sqlite3.IntegrityError: return jsonify({'error': 'Wormhole already exists or invalid system ID'}), 400
    finally: conn.close()
...
def delete_wormhole():
    data = request.get_json(); id_a, id_b = data.get('system_a_id'), data.get('system_b_id')
    if not id_a or not id_b: return jsonify({'error': 'Both system IDs are required'}), 400
    conn = get_db_connection(); cursor = conn.cursor(); cursor.execute('DELETE FROM wormholes WHERE system_a_id = ? AND system_b_id = ?', (min(id_a, id_b), max(id_a, id_b))); conn.commit(); conn.close()
...
def get_faction_systems():
    ...
    faction_id = session['faction_id']; conn = get_db_connection()
    systems_list = conn.execute('SELECT s.id, s.name, s.x, s.y, s.position, s.catapult_radius FROM systems s JOIN faction_discovered_systems fds ON s.id = fds.system_id WHERE fds.faction_id = ?', (faction_id,)).fetchall()
    systems_dict = {row['id']: dict(row) for row in systems_list}
    all_wormholes = conn.execute('SELECT system_a_id, system_b_id FROM wormholes').fetchall()
    faction_system_ids = set(systems_dict.keys())
    faction_wormholes = [(wh['system_a_id'], wh['system_b_id']) for wh in all_wormholes if wh['system_a_id'] in faction_system_ids and wh['system_b_id'] in faction_system_ids]
    conn.close()
    return jsonify({'systems': systems_dict, 'wormholes': faction_wormholes})
...
def calculate_path():
    ...
    conn = get_db_connection()
    faction_systems = conn.execute('SELECT s.id, s.position, s.catapult_radius FROM systems s JOIN faction_discovered_systems fds ON s.id = fds.system_id WHERE fds.faction_id = ?', (faction_id,)).fetchall()
    all_wormholes = conn.execute('SELECT system_a_id, system_b_id FROM wormholes').fetchall(); conn.close()
...
if __name__ == '__main__':
    ...
    # A production deployment would use a WSGI server like Gunicorn.
